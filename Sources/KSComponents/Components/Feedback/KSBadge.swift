...
#Preview("KSTag") {
    VStack(spacing: 20) {
        HStack(spacing: 10) {
            KSTag("Swift")
            KSTag("iOS", icon: "apple.logo")
            KSTag("Removable", isRemovable: true)
        }

        HStack(spacing: 10) {
            KSTag("Success", color: KS.Colors.success)
            KSTag("Warning", color: KS.Colors.warning)
            KSTag("Error", color: KS.Colors.error)
        }
    }
    .padding()
}
...
#Preview("KSChip") {
    struct ChipPreview: View {
        ...
        var body: some View {
            VStack(spacing: 20) {
                HStack(spacing: 10) {
                    KSChip("Option 1", isSelected: $isSelected1)
                    KSChip("Option 2", isSelected: $isSelected2)
                    KSChip("With Icon", icon: "star.fill", isSelected: $isSelected3)
                }
            }
            .padding()
        }
...
#Preview("KSChipGroup") {
    struct ChipGroupPreview: View {
        ...
        var body: some View {
            VStack(spacing: 20) {
                KSChipGroup(
                    chips: ["Swift", "SwiftUI", "Combine", "UIKit", "Core Data"],
                    selectedChips: $selectedChips,
                    allowMultiple: true
                )

                Text("Selected: \(selectedChips.joined(separator: ", "))")
                    .font(.caption)
            }
            .padding()
        }
